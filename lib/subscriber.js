                ...
                function (cb) {
                    if (!plans || !expiry || expiry < now) {
                        db.plans(function (err,p) {
                            ...
                            _.each(_.uniq(resources,function (item) {
                                ...
                                if (err) {
                                    ...
                                    if (u) {
                                        ...
                                        if (limit === undefined || limit === null) {
                                            ...
                                            res.status(403).send({reason:"subscription",plan:name,limit:resource,maximum:limit});
                                        } else if (expire<now) {
                                            // is the user on a trial plan, and it is expired?
                                            res.status(403).send({reason:"subscription",plan:name,expired:true,date:expire})
                                        } else {
                                            cb();
                                        }
                                    } else {
